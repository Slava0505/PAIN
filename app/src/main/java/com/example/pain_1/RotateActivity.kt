...
class RotateActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        imageForRotate.setImageURI(cururi)
        var set=false
        var origbmp = (imageForRotate.drawable as BitmapDrawable).bitmap

        buttonStart.setOnClickListener {
            if (!set)
            {
                set=true
                origbmp = (imageForRotate.drawable as BitmapDrawable).bitmap
            }
            val angle=degreesBox.text.toString().toDouble()* PI/180
            val a=dis(0.0,sin(angle*1.0),cos(angle*1.0),0.0,1.0,0.0)/(-1)
            val b=dis(0.0,0.0,1.0,0.0,sin(angle*1.0),cos(angle*1.0))/(-1)
            val c=0.0
            val d=dis(0.0,cos(angle*1.0),-sin(angle*1.0),0.0,1.0,0.0)/(-1)
            val e=dis(0.0,0.0,1.0,0.0,cos(angle*1.0),-sin(angle*1.0))/(-1)
            val f=0.0
            var i=0
            var tmaxy=f
            var tmaxx=c
            ...
            trinormal(canvasbmp)
            imageForRotate.setImageBitmap(canvasbmp)
        }

        buttonAccept.setOnClickListener {
            cururi=bitmapToFile((imageForRotate.drawable as BitmapDrawable).bitmap)
